...
export function TransactionsList({ transactions, wallets, currency, showMoreItems = false }: TransactionsListProps) {
    ...
    // Show 13 items by default, 16 when categories are expanded (to match left column height)
    const displayCount = showMoreItems ? 16 : 13;

    const renderMobileList = (data: Transaction[]) => {
        return sortData(data).slice(0, displayCount).map(t => <MobileTransactionCard key={t.id} item={t} />);
    };

    const renderList = (data: Transaction[]) => {
        return sortData(data).slice(0, displayCount).map(t => <TransactionItem key={t.id} item={t} />);
    };

    return (
        <div className="rounded-xl md:rounded-2xl bg-gradient-to-br from-amber-500/10 via-zinc-900/80 to-zinc-900/50 border border-amber-500/30 overflow-hidden h-full flex flex-col shadow-lg shadow-amber-500/5 hover:shadow-amber-500/10 transition-shadow">
            <div className="p-3 md:p-4 border-b border-amber-500/20 flex items-center justify-between bg-gradient-to-r from-amber-500/10 to-transparent">
                <div className="flex items-center gap-2 md:gap-3">
                    <div className="p-1.5 md:p-2 rounded-lg md:rounded-xl bg-gradient-to-br from-amber-500/30 to-orange-500/20 shadow-inner">
                        <Receipt className="h-3.5 w-3.5 md:h-4 md:w-4 text-amber-400" />
                    </div>
                    <h2 className="text-base md:text-lg font-bold font-serif text-white">Transactions</h2>
                </div>
                <Link to="/transactions">
                    <Button variant="ghost" size="sm" className="gap-1.5 md:gap-2 text-amber-400 hover:text-amber-300 hover:bg-amber-500/20 rounded-lg md:rounded-xl text-xs md:text-sm h-8 md:h-9 px-2 md:px-3 transition-all">
                ...
                <Tabs defaultValue="all" className="w-full">
                    ...
                    <div className="hidden md:block overflow-x-auto">
                        <div className="min-w-[700px]">
                            <div className="grid grid-cols-[2.5fr_1.2fr_1fr_1fr_1fr] gap-3 text-xs font-medium text-gray-500 mb-3 px-3">
                                <span
